...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `✅ Recherche terminée: ${results.directFlights.length} vols directs, ${results.viaAlgiersFlights.length} vols via Alger`
    );
...
export async function GET(request: NextRequest) {
  try {
    ...
    const searchService = new UnifiedFlightSearch();
    const results = await searchService.searchSpecificRoute(
      origin,
      destination,
      date
    );
