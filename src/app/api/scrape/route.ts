...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `🚀 Démarrage du scraping pour ${validatedData.origin} → ${validatedData.destination}`
    );
...
export async function GET(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Paramètres manquants: origin, destination, departureDate requis",
        },
        { status: 400 }
      );
    }
    ...
    console.log(
      `🔍 GET scraping: ${validatedData.origin} → ${validatedData.destination}`
    );
