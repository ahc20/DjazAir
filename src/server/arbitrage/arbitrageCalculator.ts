...
export class ArbitrageCalculator {
  ...
  findBestOpportunities(
    ...
    return opportunities
      ...
      .sort((a, b) => b.savingsPercentage.parallel - a.savingsPercentage.parallel);
  }
  ...
  private generateRecommendations(
    ...
    if (savings.parallel > 100) {
      recommendations.push(`💰 Opportunité exceptionnelle : ${savings.parallel}€ d'économies (${savingsPercentage.parallel}%)`);
    } else if (savings.parallel > 50) {
      recommendations.push(`✅ Bonne opportunité : ${savings.parallel}€ d'économies (${savingsPercentage.parallel}%)`);
    } else if (savings.parallel > 0) {
      recommendations.push(`⚠️ Opportunité limitée : ${savings.parallel}€ d'économies (${savingsPercentage.parallel}%)`);
    } else {
    ...
    if (route.includes('DXB') || route.includes('Dubai')) {
      recommendations.push('🌍 Vérifiez les conditions de visa pour les Émirats Arabes Unis');
      recommendations.push('✈️ Prévoyez un temps de correspondance suffisant à Alger');
    }

    if (route.includes('CDG') || route.includes('Paris')) {
      recommendations.push('🇫🇷 Départ depuis Paris - Vérifiez les horaires de correspondance');
      recommendations.push('🕐 Considérez les vols de nuit pour optimiser le temps de voyage');
    }
