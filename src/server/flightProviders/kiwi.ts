...
export class KiwiProvider implements FlightProvider {
  ...
  async searchRoundTrip(params: FlightSearchParams): Promise<FlightSearchResult> {
    ...
    try {
      ...
      if (params.returnDate) {
        searchParams.append('return_from', this.formatDateForKiwi(params.returnDate));
        searchParams.append('return_to', this.formatDateForKiwi(params.returnDate));
      }
      ...
      if (!response.ok) {
        throw new Error(`Erreur API Kiwi: ${response.status} ${response.statusText}`);
      }
      ...
      const bestDirectPrice = options.length > 0 
      ...
      throw new Error(`Erreur de recherche Kiwi: ${error instanceof Error ? error.message : 'Erreur inconnue'}`);
    }
  }

  private async searchViaAlgiers(params: FlightSearchParams): Promise<FlightSearchResult> {
  ...
  private matchesCabinClass(flight: KiwiFlight, requestedCabin?: string): boolean {
  ...
  private convertKiwiFlightToOption(flight: KiwiFlight): FlightOption {
    ...
    const totalDuration = flight.route.reduce((total, route) => total + route.duration, 0);
