...
export class AmadeusProvider implements FlightProvider {
  ...
  private async getAccessToken(): Promise<string> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Erreur d'authentification Amadeus: ${response.status}`);
      }
  ...
  async searchRoundTrip(params: FlightSearchParams): Promise<FlightSearchResult> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Erreur API Amadeus: ${response.status} ${response.statusText}`);
      }
      ...
      const options: FlightOption[] = offers.map(offer => {
        const totalDuration = offer.itineraries.reduce((total, itinerary) => {
          return total + itinerary.segments.reduce((segTotal, segment) => {
      ...
      throw new Error(`Erreur de recherche Amadeus: ${error instanceof Error ? error.message : 'Erreur inconnue'}`);
    }
  }
