...
export class UnifiedFlightSearch {
  ...
  async searchFlights(
    ...
    console.log(
      `🔍 Recherche de vols: ${params.origin} → ${params.destination} le ${params.departDate}`
    );
    ...
    return {
      ...
      bestDirectPriceEUR:
        allDirectFlights.length > 0 ? allDirectFlights[0].priceEUR : null,
      bestViaAlgiersPriceEUR:
        allViaAlgiersFlights.length > 0
          ? allViaAlgiersFlights[0].priceEUR
          : null,
      searchParams: params,
  ...
  private calculatePriceBreakdown(
    ...
    const direct = {
      ...
      totalDZD:
        directFlights.length > 0 ? directFlights[0].priceDZD : undefined,
      breakdown: directFlights.slice(0, 3), // Top 3 options
    };

    const viaAlgiers = {
      totalEUR:
        viaAlgiersFlights.length > 0 ? viaAlgiersFlights[0].priceEUR : 0,
      totalDZD:
        viaAlgiersFlights.length > 0 ? viaAlgiersFlights[0].priceDZD || 0 : 0,
