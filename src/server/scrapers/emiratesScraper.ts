...
export class EmiratesScraper extends BaseScraper implements AirlineScraper {
  ...
  async searchFlights(params: FlightSearchParams): Promise<ScrapingResult> {
    try {
      console.log(`🔍 Recherche Emirates: ${params.origin} → ${params.destination} le ${params.departureDate}`);
  ...
  private generateConnectingFlights(params: FlightSearchParams): ScrapedFlight[] {
    const flights: ScrapedFlight[] = [];
    
    // Vols avec escale via Dubai (DXB)
    if (params.origin !== 'DXB' && params.destination !== 'DXB') {
      const basePrice = this.getBasePrice(params.origin, params.destination) * 0.75; // Prix réduit pour les escales
      const departureTimes = ['08:15', '14:45', '20:30'];
      
      departureTimes.forEach((time, index) => {
        const priceVariation = 1 + (index * 0.15) + (Math.random() * 0.25 - 0.125);
        const price = basePrice * priceVariation;
        ...
        flights.push({
          ...
          destination: params.destination,
          departureTime: `${params.departureDate}T${time}:00`,
          arrivalTime: this.calculateArrivalTime(params.origin, params.destination, time, true),
          duration: this.calculateFlightDuration(params.origin, params.destination, true),
  ...
  private calculateArrivalTime(origin: string, destination: string, departureTime: string, withConnection: boolean = false): string {
    const duration = this.calculateFlightDuration(origin, destination, withConnection);
  ...
  private calculateFlightDuration(origin: string, destination: string, withConnection: boolean = false): string {
