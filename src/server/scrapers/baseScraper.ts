...
export abstract class BaseScraper {
  ...
  protected async retryWithBackoff<T>(
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        const delay =
          Math.pow(2, attempt) * this.config.delay +
          generateRandomDelay(100, 500);
        console.log(
          `Tentative ${attempt + 1} échouée pour ${this.name}, nouvelle tentative dans ${delay}ms`
        );
        await sleep(delay);
      }
    }
  ...
  protected parsePrice(priceText: string): {
    amount: number;
    currency: string;
  } {
