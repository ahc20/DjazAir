...
export class AirFranceScraper extends BaseScraper implements AirlineScraper {
  ...
  async searchFlights(params: FlightSearchParams): Promise<ScrapingResult> {
    try {
      console.log(`🔍 Recherche Air France: ${params.origin} → ${params.destination} le ${params.departureDate}`);
  ...
  private generateConnectingFlights(params: FlightSearchParams): ScrapedFlight[] {
    ...
    const hubs = ['CDG', 'ORY'];
    const basePrice = this.getBasePrice(params.origin, params.destination) * 0.8; // Prix réduit pour les escales
    ...
    hubs.forEach((hub, index) => {
      if (params.origin !== hub && params.destination !== hub) {
        ...
        flights.push({
          ...
          arrivalTime: this.calculateArrivalTime(params.origin, params.destination, index === 0 ? '09:00' : '15:30'),
