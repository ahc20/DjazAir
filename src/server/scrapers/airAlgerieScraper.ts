...
export class AirAlgerieScraper extends BaseScraper implements AirlineScraper {
  ...
  async searchFlights(params: FlightSearchParams): Promise<ScrapingResult> {
    try {
      console.log(
        `🔍 Recherche Air Algérie: ${params.origin} → ${params.destination} le ${params.departureDate}`
      );
  ...
  private generateViaAlgiersFlights(
    params: FlightSearchParams
  ): ScrapedFlight[] {
    const flights: ScrapedFlight[] = [];
    ...
    if (originToAlgiers.length > 0 && algiersToDestination.length > 0) {
      ...
      const totalPrice =
        (outbound.price?.amount || 0) + (inbound.price?.amount || 0);
      const totalDuration = this.addDurations(
        outbound.duration,
        inbound.duration
      );
