...
import { Save, Settings, Euro, Percent, Clock, AlertTriangle } from 'lucide-react';
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            {errors.eurToDzdCustomRate && (
              <p className="text-sm text-red-500">{errors.eurToDzdCustomRate.message}</p>
            )}
            <p className="text-xs text-gray-600">
              Taux utilisé pour les simulations "via Alger" quand le mode custom est sélectionné
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            {errors.minSavingsPercent && (
              <p className="text-sm text-red-500">{errors.minSavingsPercent.message}</p>
            )}
            <p className="text-xs text-gray-600">
              Seuil minimum d'économies pour qu'une option "via Alger" soit considérée comme un bon deal
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            {errors.riskBufferMinutes && (
              <p className="text-sm text-red-500">{errors.riskBufferMinutes.message}</p>
            )}
            <p className="text-xs text-gray-600">
              Temps de correspondance minimum recommandé pour les vols "via Alger"
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-600">
              Désactiver cette option masquera complètement la fonctionnalité "via Alger"
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            {errors.legalDisclaimer && (
              <p className="text-sm text-red-500">{errors.legalDisclaimer.message}</p>
            )}
            <p className="text-xs text-gray-600">
              Ce texte sera affiché sur toutes les pages pour informer les utilisateurs des limites légales
            </p>
          </div>
          ...
          <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
            <div className="text-sm text-blue-800">
              ...
              <ul className="space-y-1 text-xs">
                ...
                <li>• Le taux custom est utilisé uniquement pour les simulations</li>
                <li>• L'avertissement légal est affiché sur toutes les pages</li>
                <li>• Ces paramètres affectent le comportement de l'application pour tous les utilisateurs</li>
              </ul>
            </div>
          </div>
        </form>
